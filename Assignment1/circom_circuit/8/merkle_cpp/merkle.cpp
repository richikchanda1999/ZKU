...
void calculateHash_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void calculateHash_2_run(uint ctx_index,Circom_CalcWit* ctx);
void CalculateRoot_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void CalculateRoot_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 26605;}

uint get_number_of_components() {return 61;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 19809;}
...
ctx->componentMemory[coffset].templateName = "calculateHash";
ctx->componentMemory[coffset].signalStart = soffset;
...
ctx->componentMemory[coffset].templateName = "CalculateRoot";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 15; i++) {
std::string new_cmp_name = "hashes"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
calculateHash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[17]); // line circom 40
while(Fr_isTrue(&expaux[0])){
{
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
calculateHash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 46
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_gt(&expaux[0],&lvar[3],&circuitConstants[3]); // line circom 55
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_idiv(&expaux[0],&lvar[3],&circuitConstants[5]); // line circom 58
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_lt(&expaux[0],&lvar[2],&lvar[3]); // line circom 62
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[1])) + 0);
{
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * ((2 * Fr_toInt(&lvar[1])) + 1)) + 0)]].signalStart + 0]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
calculateHash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * ((2 * Fr_toInt(&lvar[1])) + 2)) + 0)]].signalStart + 0]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
calculateHash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_sub(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 69
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&circuitConstants[3]); // line circom 71
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[2],&lvar[3]); // line circom 62
...
CalculateRoot_3_create(1,0,ctx,"main",0);
CalculateRoot_3_run(0,ctx);
}
